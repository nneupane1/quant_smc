"""
One-command backtest room launcher (live-style dashboard + telemetry).
...
from quant_system.cli.runners.backtest_cli import main as backtest_main
...
def main() -> None:
    ...
    try:
        ...
        console_stage("Backtest run complete", f"artifacts={OUT_DIR}", status="ok")
        console_stage("Operator UI left running", f"close manually when done ({ui_desc})", status="info")
        keep_dashboard = False
        completed = True
    except KeyboardInterrupt:
        console_stage("Interrupted", "stopping backtest room", status="warn")
    finally:
        log_elapsed(started_at, "Backtest room runtime", ok=completed)
        if keep_dashboard:
