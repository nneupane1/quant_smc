...
# Safer defaults for macOS scientific stack + PyTorch joblib reloads.
...
ensure_runtime(("pandas", "numpy", "sklearn", "torch"))
...
from quant_system.ml.training.tcn_stack_trainer import TCN_STACK_TARGETS, TCNStackTrainer
    ...
    *,
    ...
    manifest = {
        ...
        "governance": {
            "submitted": False,
            "reason": (
                "TCN stack checkpoint artifact saved from latest completed challenger state; "
                "rerun the same target to continue remaining HPO."
                if outcome == "checkpoint_saved"
                else "TCN family stack model trained for routing comparison; promotion remains explicit."
            ),
...
def run_target(
    ...
    *,
    ...
    persisted = False
    target_model_name = f"{target}_tcn"
    ...
    try:
        ...
        console_rule(f"Train TCN Stack | {ASSET} | {target}", style="bright_magenta")
        console_kv(
            "TCN Stack Launch Plan",
            {
                "asset": ASSET,
                "target": target_model_name,
                "features_csv": features_csv,
                "labels_csv": labels_csv,
                "tree_manifest_source": str(Path('artifacts/train') / ASSET / target / 'train_manifest.json'),
        ...
        console_stage(
            "TCN stack training frame ready",
            f"rows={fmt_num(len(train_df))} path={merged_out}",
            status="ok",
        )

        trainer = TCNStackTrainer(
            asset=ASSET,
            target=target,
        ...
        version = versioner.new_version(f"{ASSET}_tcn_stack")
        dependency_models = [f"{name}_tcn" for name in trainer.required_specialists()]
        manifest = _persist_tcn_stack_result(
            asset=ASSET,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train BTCUSD TCN stack model over trained TCN specialists.")
    parser.add_argument(
        "--target",
        default="meta_model",
        choices=sorted(TCN_STACK_TARGETS.keys()),
        help="TCN stack target to train.",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
    ...
    run_target(args.target, resume=not args.no_resume)
