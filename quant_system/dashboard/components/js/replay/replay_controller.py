...
from quant_system.execution.hazard_trailing import HazardTrailing
from quant_system.execution.mpc_risk import MPCRiskEngine
...
class ReplayController:
    def __init__(
        ...
        self.hazard_engine = HazardTrailing(config)
        self.mpc_risk = MPCRiskEngine(config)
    ...
    def render_payload(self) -> Dict[str, Any]:
        ...
        conf = self.confluence.compute(prob_dict, smc_row, row, self.session.classify(row["timestamp"]))
        evr = self.evr_calc.compute(prob_dict, row)
        hazard = self.hazard_engine.estimate_hazard(prob_dict, row)

        # -----------------------------------------------------
        # 5) MPC Risk State
        # -----------------------------------------------------
        risk_state = self.mpc_risk.compute_from_snapshot(prob_dict, row)
