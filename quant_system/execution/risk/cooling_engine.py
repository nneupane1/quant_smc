...
class CoolingEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ccfg = config["execution"]["cooling"]

        self.min_bars = int(ccfg["min_bars"])          # e.g., 4
        self.max_bars = int(ccfg["max_bars"])          # e.g., 20

        # Threshold increases during cooling
        self.conf_boost = float(ccfg["conf_boost"])    # e.g., +0.15
        self.evr_boost  = float(ccfg["evr_boost"])     # e.g., +0.30
        self.hazard_cut = float(ccfg["hazard_cut"])    # e.g., -0.10

        # Override parameters
        self.moon_conf_add  = float(ccfg["moon_conf_add"])    # +0.10
        self.moon_evr_min   = float(ccfg["moon_evr_min"])     # 1.80
        self.moon_medR_min  = float(ccfg["moon_medR_min"])    # 6.0
        self.moon_hazard_max = float(ccfg["moon_hazard_max"]) # 0.25

        self.twoR_conf_add = float(ccfg["twoR_conf_add"])      # +0.05
        self.twoR_evr_min  = float(ccfg["twoR_evr_min"])       # 1.20
        self.twoR_medR_min = float(ccfg["twoR_medR_min"])      # 2.0
        self.twoR_hazard_max = float(ccfg["twoR_hazard_max"])  # 0.40
